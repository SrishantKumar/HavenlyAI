export const AI_CONFIG = {
  textModel: 'gemini-flash-latest',      // Fast, multimodal audio & text model (Gemini 3.8 Flash)
...
export const CONFIG = {
  // Demo mode defaults to true only when neither Gemini nor OpenRouter key is configured
  ...
  get openRouterApiKey(): string {
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        const stored = window.localStorage.getItem('havenly_openrouter_api_key');
